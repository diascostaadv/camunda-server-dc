...
class BuscarPublicacoesRequest(BaseModel):
    ...
    data_final: Optional[str] = Field(
        None, description="Data final para busca (YYYY-MM-DD)"
    )

    # Configurações de processamento
    limite_publicacoes: int = Field(
        default=50, ge=1, le=1000, description="Limite de publicações a processar"
    )
    timeout_soap: int = Field(
        default=90, ge=30, le=300, description="Timeout para chamadas SOAP em segundos"
    )
    max_retries: int = Field(
        default=3, ge=1, le=10, description="Máximo de tentativas em caso de erro"
    )

    # Configurações do processo Camunda
    process_key: str = Field(
        default="processar_movimentacao_judicial",
        description="Chave do processo a iniciar",
    )
    usar_business_key: bool = Field(
        default=True, description="Se deve usar business key nas instâncias"
    )

    # Filtros opcionais
    filtrar_duplicatas: bool = Field(
        default=True, description="Se deve filtrar publicações duplicadas"
    )
    apenas_nao_exportadas: bool = Field(
        default=False, description="Se deve buscar apenas não exportadas"
    )
...
class PublicacaoProcessingResult(BaseModel):
    ...
    numero_processo: Optional[str] = Field(
        None, description="Número do processo judicial"
    )
    status: Literal["success", "error", "skipped"] = Field(
        ..., description="Status do processamento"
    )
    instance_id: Optional[str] = Field(
        None, description="ID da instância do processo criada"
    )
...
class BuscarPublicacoesResponse(BaseModel):
    ...
    total_encontradas: int = Field(
        ..., description="Total de publicações encontradas na API SOAP"
    )
    total_processadas: int = Field(
        ..., description="Total de publicações que tentaram ser processadas"
    )
    total_filtradas: int = Field(
        ..., description="Total de publicações filtradas/ignoradas"
    )

    # Estatísticas do processamento Camunda
    instancias_criadas: int = Field(
        ..., description="Número de instâncias de processo criadas"
    )
    instancias_com_erro: int = Field(
        ..., description="Número de instâncias que falharam"
    )
    instancias_ignoradas: int = Field(
        ..., description="Número de publicações ignoradas"
    )
    ...
    resultados_detalhados: List[PublicacaoProcessingResult] = Field(
        default_factory=list, description="Resultados detalhados por publicação"
    )

    # Informações da configuração utilizada
    configuracao_utilizada: Dict[str, Any] = Field(
        default_factory=dict, description="Configuração que foi utilizada na busca"
    )

    # Erros gerais
    erros_gerais: List[str] = Field(
        default_factory=list, description="Erros gerais durante a execução"
    )

    # Estatísticas adicionais
    publicacoes_por_tribunal: Dict[str, int] = Field(
        default_factory=dict, description="Contagem de publicações por tribunal"
    )
    publicacoes_por_fonte: Dict[str, int] = Field(
        default_factory=dict, description="Contagem de publicações por fonte"
    )
...
class BuscarPublicacoesStatus(BaseModel):
    ...
    timestamp_ultima_atualizacao: datetime = Field(
        default_factory=datetime.now, description="Última atualização"
    )

    # Progresso
    total_esperado: Optional[int] = Field(
        None, description="Total de itens esperados para processar"
    )
    total_processado: int = Field(
        default=0, description="Total de itens já processados"
    )
    progresso_percentual: Optional[float] = Field(
        None, description="Progresso em percentual (0.0 a 1.0)"
    )
    ...
    mensagem_status: str = Field(
        default="", description="Mensagem descritiva do status"
    )

    # Resultado final (quando completo)
    resultado_final: Optional[BuscarPublicacoesResponse] = Field(
        None, description="Resultado final quando completado"
    )
    ...
    def atualizar_progresso(
        ...
        if self.total_esperado and self.total_esperado > 0:
            self.progresso_percentual = min(
                1.0, self.total_processado / self.total_esperado
            )
...
class TaskDataRequest(BaseModel):
    ...
    variables: Dict[str, Any] = Field(
        default_factory=dict, description="Variáveis da tarefa"
    )
...
class PublicacaoParaProcessamento(BaseModel):
    ...
    @classmethod
    def from_soap_publicacao(
        cls, publicacao, fonte: str = "dw"
        ...
        if hasattr(publicacao, "descricao_diario") and publicacao.descricao_diario:
            tribunal = (
                publicacao.descricao_diario.lower().replace(" ", "").replace("-", "")
            )
        ...
        texto_publicacao = (
            ...
            or publicacao.despacho_publicacao
            or publicacao.processo_publicacao
            or f"Publicação código {publicacao.cod_publicacao}"
        )
