...
class PublicacaoBronze(BaseModel):
    ...
    data_publicacao: str = Field(..., description="Data de publicação (formato original)")
    texto_publicacao: str = Field(..., description="Texto integral da publicação")
    fonte: Literal["dw", "manual", "escavador"] = Field(..., description="Fonte da publicação")
    tribunal: str = Field(..., description="Tribunal de origem")
    instancia: str = Field(..., description="Instância judicial")
    descricao_diario: Optional[str] = Field(None, description="Descrição do diário oficial")
    uf_publicacao: Optional[str] = Field(None, description="UF da publicação")
    timestamp_insercao: datetime = Field(default_factory=datetime.now, description="Timestamp de inserção")
    status: Literal["nova", "processada", "repetida", "erro"] = Field(default="nova", description="Status do processamento")
...
class PublicacaoPrata(BaseModel):
    ...
    publicacao_bronze_id: str = Field(..., description="ID da publicação bronze original")
    hash_unica: str = Field(..., description="Hash SHA256 única para deduplicação")
    hash_alternativa: Optional[str] = Field(None, description="Hash alternativa com texto limpo")
    
    # Dados principais higienizados
    numero_processo: str = Field(..., description="Número do processo original")
    numero_processo_limpo: str = Field(..., description="Número do processo normalizado")
    data_publicacao: datetime = Field(..., description="Data de publicação parseada")
    data_publicacao_original: str = Field(..., description="Data de publicação formato original")
    ...
    status: Literal[
        "nova_publicacao_inedita",
        "identidade_duvidosa", 
        "repetida",
        "apto_a_agenda"
    ] = Field(..., description="Status após deduplicação")
    
    score_similaridade: float = Field(0.0, ge=0.0, le=100.0, description="Score de similaridade (0-100)")
    
    # Publicações similares encontradas
    publicacoes_similares: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Lista de publicações similares com scores"
    )
    
    # Classificação
    classificacao: Optional[Dict[str, Any]] = Field(None, description="Classificação da publicação")
    
    # Controle
    timestamp_processamento: datetime = Field(default_factory=datetime.now, description="Timestamp do processamento")
    camunda_instance_id: Optional[str] = Field(None, description="ID da instância no Camunda")
...
class Lote(BaseModel):
    ...
    timestamp_criacao: datetime = Field(default_factory=datetime.now, description="Timestamp de criação")
    ...
    publicacoes_ids: List[str] = Field(default_factory=list, description="IDs das publicações do lote")
...
class HashDeduplicacao(BaseModel):
    ...
    numero_processo: str = Field(..., description="Número do processo para referência rápida")
...
class ClassificacaoPublicacao(BaseModel):
    ...
    # Entidades extraídas
    partes_mencionadas: List[str] = Field(default_factory=list, description="Partes mencionadas no texto")
    advogados_mencionados: List[str] = Field(default_factory=list, description="Advogados mencionados")
    
    # Análise de conteúdo
    palavras_chave: List[str] = Field(default_factory=list, description="Palavras-chave identificadas")
    ...
    confianca: float = Field(0.0, ge=0.0, le=1.0, description="Nível de confiança (0-1)")
...
class ResultadoDeduplicacao(BaseModel):
    ...
    # Se for duplicata
    publicacao_original_id: Optional[str] = Field(None, description="ID da publicação original se duplicata")
    score_similaridade: float = Field(0.0, description="Score de similaridade")
    
    # Publicações similares encontradas
    publicacoes_similares: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Lista de publicações similares"
    )
    ...
    timestamp_analise: datetime = Field(default_factory=datetime.now, description="Timestamp da análise")
...
class ProcessamentoPublicacaoRequest(BaseModel):
    ...
    publicacao_bronze_id: str = Field(..., description="ID da publicação bronze a processar")
    executar_classificacao: bool = Field(True, description="Se deve executar classificação")
    executar_deduplicacao: bool = Field(True, description="Se deve executar deduplicação")
    iniciar_processo_camunda: bool = Field(False, description="Se deve iniciar processo no Camunda")
    ...
    score_minimo_similaridade: float = Field(
        ...
        ge=0.0,
        le=100.0,
        description="Score mínimo para considerar duplicata"
    )
...
class ProcessamentoLoteRequest(BaseModel):
    ...
    processar_em_paralelo: bool = Field(True, description="Se deve processar em paralelo")
    max_paralelo: int = Field(10, ge=1, le=50, description="Máximo de processamentos paralelos")
    continuar_em_erro: bool = Field(True, description="Se deve continuar processando em caso de erro")
    
    # Configurações de processamento
    executar_classificacao: bool = Field(True, description="Se deve executar classificação")
    executar_deduplicacao: bool = Field(True, description="Se deve executar deduplicação")
    iniciar_processos_camunda: bool = Field(False, description="Se deve iniciar processos no Camunda")
...
class ProcessamentoLoteResponse(BaseModel):
    ...
    # Estatísticas
    total_publicacoes: int = Field(..., description="Total de publicações no lote")
    processadas_sucesso: int = Field(..., description="Publicações processadas com sucesso")
    ...
    publicacoes_prata_ids: List[str] = Field(
        default_factory=list,
        description="IDs das publicações prata criadas"
    )
