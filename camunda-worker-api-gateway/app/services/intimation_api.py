...
class IntimationAPIClient:
        ...
        for tentativa in range(self.max_retries + 1):
            try:
                if tentativa > 0:
                    ...
                    logger.info(f"Tentativa {tentativa + 1}/{self.max_retries + 1} para {method}. Aguardando {wait_time}s...")
                    time.sleep(wait_time)
                
                logger.debug(f"Fazendo requisição para {method} (tentativa {tentativa + 1})")
                ...
                logger.debug(f"Requisição {method} bem-sucedida após {tentativa + 1} tentativa(s)")
                return self._parse_soap_response(response.text)
                
            except requests.exceptions.Timeout as e:
                last_exception = e
                logger.warning(f"Timeout na tentativa {tentativa + 1} para {method}: {e}")
                if tentativa == self.max_retries:
                    break
                    
            except requests.exceptions.ConnectionError as e:
                last_exception = e
                logger.warning(f"Erro de conexão na tentativa {tentativa + 1} para {method}: {e}")
                if tentativa == self.max_retries:
                    break
                    
            except requests.exceptions.HTTPError as e:
                last_exception = e
                logger.error(f"Erro HTTP na tentativa {tentativa + 1} para {method}: {e}")
                ...
                last_exception = e
                logger.error(f"Erro inesperado na tentativa {tentativa + 1} para {method}: {e}")
                if tentativa == self.max_retries:
                    break
        
        # Se chegou aqui, todas as tentativas falharam
        logger.error(f"Todas as {self.max_retries + 1} tentativas falharam para {method}")
        raise last_exception or requests.RequestException(f"Falha após {self.max_retries + 1} tentativas")
    ...
    def _parse_soap_response(self, response_text: str) -> ET.Element:
        ...
        try:
            ...
            logger.error(f"Erro ao fazer parse da resposta XML: {e}")
            raise
    
    def get_publicacoes_nao_exportadas(self, cod_grupo: int = 0) -> List[Publicacao]:
        """
        Retorna todas as intimações ainda não marcadas como exportadas
        
        Args:
            cod_grupo: Filtra as intimações por grupo (0 - padrão)
    ...
    def get_publicacoes_nao_exportadas_v(self, cod_grupo: int = 0, versao: int = 5) -> List[Publicacao]:
        """
    ...
    def get_publicacoes(self, data_inicial: str, data_final: str, 
        ...
        """
        Retorna todas as intimações do período informado
    ...
    def get_publicacoes_v(self, data_inicial: str, data_final: str, 
                         cod_grupo: int = 0, exportada: int = 0, versao: int = 5) -> List[Publicacao]:
        """
        Versão com especificação de versão dos dados de retorno
    ...
    def set_publicacoes(self, codigos_publicacao: List[int]) -> bool:
        """
        Marca as intimações como "exportadas"
    ...
    def set_publicacoes_grupo(self, codigos_publicacao: List[int], cod_grupo: int = 0) -> bool:
        """
        Marca as intimações como "exportadas" por grupo
    ...
    def get_estatisticas_publicacoes(self, data: str, cod_grupo: int = 0, 
                                   tipo_agrupamento: str = "", versao: int = 1) -> EstatisticasPublicacoes:
        """
        Retorna estatísticas de publicações de uma determinada data
        
        Args:
            data: Data no formato yyyy-mm-dd
            cod_grupo: Grupo de publicações a serem consideradas
    ...
    def _parse_publicacoes(self, root: ET.Element) -> List[Publicacao]:
        ...
        try:
            ...
            logger.debug(f"Encontrados {len(publicacao_elements)} elementos de publicação")
            ...
            for pub_elem in publicacao_elements:
                publicacao = Publicacao(
                    ...
                    oab_numero=self._get_int_value_ns(pub_elem, 'oABNumero', namespaces),  # Note: oABNumero, não OABNumero
                    oab_estado=self._get_text_value_ns(pub_elem, 'oABEstado', namespaces),  # Note: oABEstado, não OABEstado
            ...
            logger.info(f"Parsed {len(publicacoes)} publicações do XML")
            return publicacoes
            
        except Exception as e:
            logger.error(f"Erro ao fazer parse das publicações: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return []
    
    def _get_text_value(self, element: ET.Element, tag_name: str) -> str:
        """Extrai valor de texto de um elemento XML"""
        elem = element.find(tag_name)
        return elem.text.strip() if elem is not None and elem.text else ""
    
    def _get_int_value(self, element: ET.Element, tag_name: str) -> int:
        """Extrai valor inteiro de um elemento XML"""
        text_value = self._get_text_value(element, tag_name)
        try:
            return int(text_value) if text_value else 0
        except ValueError:
            return 0
    
    def _get_text_value_ns(self, element: ET.Element, tag_name: str, namespaces: dict) -> str:
    ...
    def _get_int_value_ns(self, element: ET.Element, tag_name: str, namespaces: dict) -> int:
    ...
    def _parse_boolean_response(self, root: ET.Element) -> bool:
        ...
        try:
            ...
            logger.error(f"Erro ao fazer parse da resposta booleana: {e}")
            return False
        ...
        try:
            ...
            if stats_elem is not None:
                return EstatisticasPublicacoes(
                    grupo=self._get_text_value(stats_elem, 'grupo'),
                    total_publicacoes=self._get_int_value(stats_elem, 'totalPublicacoes'),
            ...
            logger.error(f"Erro ao fazer parse das estatísticas: {e}")
            return EstatisticasPublicacoes()
    
    def importar_publicacoes_rotina(self, cod_grupo: int = 0, max_iteracoes: int = 50) -> List[Publicacao]:
        """
        ...
        while iteracao < max_iteracoes:
            iteracao += 1
            logger.info(f"Iteração {iteracao}: Buscando publicações não exportadas")
            
            # Busca próximo lote
            publicacoes = self.get_publicacoes_nao_exportadas(cod_grupo)
            ...
            if len(publicacoes) < 700:
                logger.info("Último lote de publicações processado")
                
                # Marcar como exportadas
                codigos = [pub.cod_publicacao for pub in publicacoes]
                if self.set_publicacoes(codigos):
                    logger.info(f"Marcadas {len(codigos)} publicações como exportadas")
                else:
        ...
        logger.info(f"Rotina finalizada. Total de publicações importadas: {len(todas_publicacoes)}")
        return todas_publicacoes
    
    def publicacoes_to_json(self, publicacoes: List[Publicacao]) -> str:
        """Converte lista de publicações para JSON"""
        return json.dumps([pub.to_dict() for pub in publicacoes], ensure_ascii=False, indent=2)
    
    def publicacoes_to_dict(self, publicacoes: List[Publicacao]) -> List[Dict[str, Any]]:
        """Converte lista de publicações para lista de dicionários"""
        return [pub.to_dict() for pub in publicacoes]
    ...
    def get_publicacoes_periodo_safe(self, data_inicial: str, data_final: str, 
        ...
        if timeout_override:
            ...
            logger.info(f"Timeout aumentado para {timeout_override}s para período {data_inicial} - {data_final}")
        
        try:
            # Usa versão V para melhor compatibilidade
            publicacoes = self.get_publicacoes_v(
            ...
            logger.info(f"Encontradas {len(publicacoes)} publicações no período {data_inicial} - {data_final}")
            return publicacoes
            ...
            if timeout_override:
                ...
                logger.debug(f"Timeout restaurado para {timeout_original}s")
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            stats = self.get_estatisticas_publicacoes(hoje, cod_grupo=0)
...
# Exemplo de uso para testes com dados de homologação
if __name__ == "__main__":
    ...
    try:
        # Teste de importação de publicações
        print("=== Teste: Importação de publicações ===")
        publicacoes = client.get_publicacoes_nao_exportadas(cod_grupo=5)
        print(f"Encontradas {len(publicacoes)} publicações")
        
        if publicacoes:
            # Mostra a primeira publicação como exemplo
            primeira_pub = publicacoes[0]
            print("\n=== Primeira publicação ===")
            print(f"Código: {primeira_pub.cod_publicacao}")
            print(f"Processo: {primeira_pub.numero_processo}")
            print(f"UF: {primeira_pub.uf_publicacao}")
            print(f"Data Publicação: {primeira_pub.data_publicacao}")
            
            # Converte para JSON
            print("\n=== Conversão para JSON ===")
            json_output = primeira_pub.to_json()
            print(json_output)
            
            # Converte todas para JSON
            print(f"\n=== Todas as {len(publicacoes)} publicações em JSON ===")
            all_json = client.publicacoes_to_json(publicacoes)
            
            # Salva em arquivo para análise
            with open('publicacoes.json', 'w', encoding='utf-8') as f:
                f.write(all_json)
            print("Salvo em publicacoes.json")
