...
class IntimationAPIClient:
    ...
    def _make_request(self, method: str, params: Dict[str, Any]) -> Any:
        ...
        for tentativa in range(self.max_retries + 1):
            try:
                if tentativa > 0:
                    ...
                    logger.info(
                        f"Tentativa {tentativa + 1}/{self.max_retries + 1} para {method}. Aguardando {wait_time}s..."
                    )
                    time.sleep(wait_time)

                logger.debug(
                    f"Fazendo requisição para {method} (tentativa {tentativa + 1})"
                )
                ...
                logger.debug(
                    f"Requisição {method} bem-sucedida após {tentativa + 1} tentativa(s)"
                )
                ...
                last_exception = e
                logger.warning(
                    f"Timeout na tentativa {tentativa + 1} para {method}: {e}"
                )
                ...
                last_exception = e
                logger.warning(
                    f"Erro de conexão na tentativa {tentativa + 1} para {method}: {e}"
                )
                if tentativa == self.max_retries:
                    break
                ...
                last_exception = e
                logger.error(
                    f"Erro HTTP na tentativa {tentativa + 1} para {method}: {e}"
                )
                ...
                last_exception = e
                logger.error(
                    f"Erro inesperado na tentativa {tentativa + 1} para {method}: {e}"
                )
                if tentativa == self.max_retries:
                    break
        ...
        logger.error(
            f"Todas as {self.max_retries + 1} tentativas falharam para {method}"
        )
    ...
    def get_publicacoes(
        self, data_inicial: str, data_final: str, cod_grupo: int = 0, exportada: int = 0
    ) -> List[Publicacao]:
        """
    ...
    def set_publicacoes_grupo(
        self, codigos_publicacao: List[int], cod_grupo: int = 0
    ) -> bool:
        """
        ...
        params = {"intCodGrupo": cod_grupo, "strPublicacoes": codigos_str}
    ...
    def _parse_publicacoes(self, root: ET.Element) -> List[Publicacao]:
        ...
        try:
            ...
            logger.debug(
                f"Encontrados {len(publicacao_elements)} elementos de publicação"
            )
    ...
    def _get_text_value_ns(
        self, element: ET.Element, tag_name: str, namespaces: dict
    ) -> str:
        ...
        try:
            ...
            if stats_elem is not None:
                return EstatisticasPublicacoes(
                    ...
                    total_publicacoes=self._get_int_value(
                        stats_elem, "totalPublicacoes"
                    ),
                    total_nao_importadas=self._get_int_value(
    ...
    def importar_publicacoes_rotina(
        self, cod_grupo: int = 0, max_iteracoes: int = 50
    ) -> List[Publicacao]:
        """
        ...
        logger.info(
            f"Rotina finalizada. Total de publicações importadas: {len(todas_publicacoes)}"
        )
    ...
    def publicacoes_to_json(self, publicacoes: List[Publicacao]) -> str:
        ...
        return json.dumps(
            [pub.to_dict() for pub in publicacoes], ensure_ascii=False, indent=2
        )
    ...
    def get_publicacoes_periodo_safe(
        ...
        if timeout_override:
            ...
            logger.info(
                f"Timeout aumentado para {timeout_override}s para período {data_inicial} - {data_final}"
            )
        ...
        try:
            ...
            logger.info(
                f"Encontradas {len(publicacoes)} publicações no período {data_inicial} - {data_final}"
            )
            return publicacoes
...
if __name__ == "__main__":
    ...
    client = IntimationAPIClient(usuario="100049", senha="DcDpW@24")
    ...
    try:
        ...
        print("\n=== Teste: Estatísticas ===")
        stats = client.get_estatisticas_publicacoes("2025-05-01", cod_grupo=2)
