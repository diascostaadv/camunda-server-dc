...
from models.publicacao import PublicacaoPrata, ResultadoDeduplicacao, HashDeduplicacao
...
class DeduplicacaoService:
    ...
    def __init__(
        self, mongo_client: MongoClient, database_name: str = "camunda_publicacoes"
    ):
        """
        ...
        self.score_minimo_duplicata = (
            90.0  # Score mínimo para considerar duplicata exata
        )
    ...
    def verificar_duplicata(
        self, publicacao: PublicacaoPrata, score_minimo: float = None
        ...
        try:
            logger.info(
                f"Verificando duplicata para hash {publicacao.hash_unica[:16]}..."
            )
            ...
            if hash_existente:
                ...
                return self._criar_resultado_duplicata_exata(publicacao, hash_existente)

            # 2. Buscar publicações do mesmo processo
            publicacoes_processo = self._buscar_publicacoes_processo(
                publicacao.numero_processo_limpo, publicacao.data_publicacao
            )

            if not publicacoes_processo:
                # Nenhuma publicação do processo - é inédita
                logger.info(
                    f"✅ Nenhuma publicação encontrada para o processo - publicação inédita"
                )
                return self._criar_resultado_inedita(publicacao)

            # 3. Calcular similaridade com publicações existentes
            logger.info(
                f"Calculando similaridade com {len(publicacoes_processo)} publicações do processo"
            )
    ...
    def _buscar_publicacoes_processo(
        self, numero_processo: str, data_publicacao: datetime
    ) -> List[Dict[str, Any]]:
        """
        ...
        query = {
            ...
            "data_publicacao": {"$gte": data_inicio, "$lte": data_fim},
        }

        publicacoes = list(self.col_publicacoes_prata.find(query))
        logger.debug(
            f"Encontradas {len(publicacoes)} publicações para o processo {numero_processo}"
        )
    ...
    def _calcular_similaridade_multipla(
        ...
        for pub_existente in publicacoes_existentes:
            ...
            score = self._calcular_score_fuzzy(
                publicacao.texto_limpo, pub_existente.get("texto_limpo", "")
            )
                ...
                publicacoes_similares.append(
                    {
                        ...
                        "numero_processo": pub_existente.get(
                            "numero_processo_limpo",
                            pub_existente.get("numero_processo"),
                        ),
        ...
        if maior_score >= self.score_minimo_duplicata:
            status = "repetida"
            justificativa = (
                f"Publicação com similaridade de {maior_score:.1f}% encontrada"
            )
        elif maior_score >= self.score_minimo_similar:
            status = "identidade_duvidosa"
            justificativa = (
                f"Publicações similares encontradas (maior score: {maior_score:.1f}%)"
            )
        else:
        ...
        return ResultadoDeduplicacao(
            ...
            publicacao_original_id=(
                str(publicacao_mais_similar["_id"]) if publicacao_mais_similar else None
            ),
    ...
    def _criar_resultado_duplicata_exata(
        self, publicacao: PublicacaoPrata, hash_doc: Dict[str, Any]
    ) -> ResultadoDeduplicacao:
        """
        ...
        publicacoes_com_mesma_hash = list(
            self.col_hashes.find({"hash_value": publicacao.hash_unica})
        )
        ...
        for doc in publicacoes_com_mesma_hash:
            ...
            pub_prata = None
            try:
                # Tentar usar como ObjectId se for válido
                if len(doc["publicacao_prata_id"]) == 24 and all(
                    c in "0123456789abcdefABCDEF" for c in doc["publicacao_prata_id"]
                ):
                    pub_prata = self.col_publicacoes_prata.find_one(
                        {"_id": ObjectId(doc["publicacao_prata_id"])}
                    )
                else:
                    # Se não for ObjectId válido, buscar por cod_publicacao
                    pub_prata = self.col_publicacoes_prata.find_one(
                        {"cod_publicacao": int(doc["publicacao_prata_id"])}
                    )
            except (ValueError, TypeError) as e:
                logger.warning(
                    f"Erro ao processar publicacao_prata_id '{doc['publicacao_prata_id']}': {e}"
                )
                # Buscar por cod_publicacao como fallback
                try:
                    pub_prata = self.col_publicacoes_prata.find_one(
                        {"cod_publicacao": int(doc["publicacao_prata_id"])}
                    )
                    ...
                    logger.error(
                        f"Não foi possível processar publicacao_prata_id: {doc['publicacao_prata_id']}"
                    )
                    continue
        ...
        publicacoes_similares.sort(
            key=lambda x: x.get("data_publicacao", datetime.min), reverse=True
        )
    ...
    def _criar_resultado_inedita(
        self, publicacao: PublicacaoPrata
    ) -> ResultadoDeduplicacao:
        """
    ...
    def buscar_publicacoes_similares(
        self, texto: str, limite: int = 10, score_minimo: float = 70.0
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            publicacoes = (
                self.col_publicacoes_prata.find(
    ...
    def atualizar_status_duplicata(
        ...
        try:
            ...
            result = self.col_publicacoes_prata.update_one(
                {"_id": ObjectId(publicacao_id)}, update_doc
            )
