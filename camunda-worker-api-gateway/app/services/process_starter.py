...
class ProcessStarter:
    ...
    def _format_variables_for_camunda(self, variables: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def start_process_instance(self, process_key: str, variables: Dict[str, Any] = None,
                             business_key: str = None) -> Optional[str]:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Processo {process_key} iniciado com sucesso - Instance ID: {instance_id}")
    ...
    def start_movimentacao_process(self, movimentacao_data: Dict[str, Any], 
                                  business_key: str = None) -> Optional[str]:
        """
        ...
        required_fields = [
            "numero_processo", "data_publicacao", "texto_publicacao", 
            "fonte", "tribunal", "instancia"
        ]
        
        missing_fields = [field for field in required_fields if not movimentacao_data.get(field)]
        if missing_fields:
    ...
    def start_multiple_processes(self, process_key: str, 
                               variables_list: List[Dict[str, Any]],
                               business_key_field: str = None) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(f"Lote processado: {success_count} sucessos, {error_count} erros de {len(variables_list)} total")
        
        return results
    
    def start_movimentacoes_batch(self, movimentacoes_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def get_process_instance_status(self, instance_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Erro ao obter status da instância {instance_id}: HTTP {response.status_code}")
                return None
    ...
    def get_process_statistics(self, process_key: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            active_count = len(response_active.json()) if response_active.status_code == 200 else 0
            ...
            history_data = response_history.json() if response_history.status_code == 200 else []
            
            completed_count = len([h for h in history_data if h.get('state') == 'COMPLETED'])
            terminated_count = len([h for h in history_data if h.get('state') in ['EXTERNALLY_TERMINATED', 'INTERNALLY_TERMINATED']])
...
def get_process_starter(camunda_url: str = None, username: str = None, password: str = None) -> ProcessStarter:
    """
