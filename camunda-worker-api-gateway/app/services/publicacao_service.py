...
from models.publicacao import PublicacaoPrata
...
class PublicacaoService:
    ...
    def higienizar_publicacao(
        self, publicacao_bronze: Dict[str, Any]
    ) -> PublicacaoPrata:
        """
        ...
        Args:
            publicacao_bronze: Dicionário com dados da publicação bronze

        Returns:
            PublicacaoPrata: Publicação higienizada
        """
        try:
            cod_publicacao = publicacao_bronze.get("cod_publicacao")
            logger.info(f"Iniciando higienização da publicação {cod_publicacao}")
            ...
            numero_processo_limpo = self._limpar_numero_processo(
                publicacao_bronze.get("numero_processo", "")
            )

            # 3. Processar data
            data_publicacao_processada = self._processar_data(
                publicacao_bronze.get("data_publicacao", "")
            )
            ...
            logger.info(f"✅ Publicação {cod_publicacao} higienizada com sucesso")
            return publicacao_prata

        except Exception as e:
            logger.error(
                f"Erro ao higienizar publicação {publicacao_bronze.get('cod_publicacao', 'N/A')}: {e}"
            )
            raise
    ...
    def _higienizar_texto(self, texto: str) -> str:
        ...
        for padrao in self.cabecalhos_fornecedor:
            texto_limpo = re.sub(
                padrao, "", texto_limpo, flags=re.IGNORECASE | re.MULTILINE
            )
    ...
    def _processar_data(self, data_str: str) -> datetime:
        ...
        logger.warning(
            f"Não foi possível processar data: {data_str}. Usando data atual."
        )
        return datetime.now()
