...
class LoteService:
    ...
    def __init__(
        ...
        self.deduplicacao_service = deduplicacao_service or DeduplicacaoService(mongo_client, database_name)
    ...
    def _salvar_publicacoes_bronze(self, lote_id: str, publicacoes: List[Dict[str, Any]]):
        """
        ...
        try:
            ...
            if documentos_bronze:
                ...
                self.col_lotes.update_one(
                    ...
                    {'$set': {'publicacoes_ids': [str(id) for id in result.inserted_ids]}}
                )
    ...
    def processar_lote(
        ...
        try:
            ...
            self.col_lotes.update_one(
                {'_id': ObjectId(lote_id)},
                {'$set': {'status': 'processando', 'timestamp_inicio_processamento': timestamp_inicio}}
            )
            
            # Busca publicações bronze do lote
            publicacoes_bronze = list(self.col_publicacoes_bronze.find({
                'lote_id': lote_id,
    ...
    def _processar_publicacao(
        ...
        try:
            ...
            if executar_deduplicacao:
                resultado_dedup = self.deduplicacao_service.verificar_duplicata(pub_prata)
            ...
            if executar_classificacao:
                tipo, confianca = self.publicacao_service.identificar_tipo_publicacao(pub_prata.texto_limpo)
                urgente, prazo = self.publicacao_service.calcular_urgencia(pub_prata.texto_limpo, tipo)
                entidades = self.publicacao_service.extrair_entidades(pub_prata.texto_limpo)
            ...
            logger.error(f"Erro ao processar publicação {pub_bronze_doc.get('cod_publicacao')}: {e}")
            return {
                'success': False,
                'publicacao_bronze_id': str(pub_bronze_doc['_id']),
                'erro': str(e)
    ...
    def _atualizar_estatisticas_lote(
        ...
        self.col_lotes.update_one(
            ...
            {
                '$set': {
                    'status': 'processado' if total_erro == 0 else 'processado_com_erros',
    ...
    def _criar_resposta_vazia(self, lote_id: str, timestamp_inicio: datetime) -> ProcessamentoLoteResponse:
