...
class GatewayClient:
    ...
    def __init__(self, worker_id: str):
        ...
        self.communication_mode = os.getenv(
        ...
        logger.info(
            f"Initialized Gateway Client for worker {worker_id} using {self.communication_mode} mode"
        )
    ...
    def get_task_status(self, task_id: str) -> Optional[TaskStatus]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.gateway_url}/tasks/{task_id}/status", timeout=self.http_timeout
            )
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to get task status {task_id}: HTTP {response.status_code}"
                )
                return None
    ...
    def retry_task(self, task_id: str) -> bool:
        ...
        try:
            response = requests.post(
                f"{self.gateway_url}/tasks/{task_id}/retry", timeout=self.http_timeout
            )
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to retry task {task_id}: HTTP {response.status_code}"
                )
                return False
    ...
    def _submit_via_http(self, task_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(
                    f"Successfully submitted task {task_data['task_id']} via HTTP"
                )
                return True
            else:
                logger.error(
                    f"Failed to submit task via HTTP: {response.status_code} - {response.text}"
                )
                return False
    ...
    def _submit_via_rabbitmq(self, task_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully submitted task {task_data['task_id']} via RabbitMQ"
            )
            return True
    ...
    def _ensure_rabbitmq_connection(self) -> bool:
        ...
        try:
            ...
            self._rabbitmq_channel.exchange_declare(
                exchange=self.rabbitmq_exchange, exchange_type="topic", durable=True
            )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"gateway_status": "unreachable", "error": str(e)}
